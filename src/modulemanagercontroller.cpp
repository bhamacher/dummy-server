...
void ModuleManagerController::initializeEntity(const QString &t_sessionPath, const QStringList &t_sessionList)
{
    if(m_storageSystem!=nullptr)
    {
        ...
        QList<int> ents = m_storageSystem->getEntityList();
        ents.append(3);
        initData->setNewValue(QVariant::fromValue<QList<int> >(ents));
...
void ModuleManagerController::initOnce()
{
    if(m_initDone == false)
    {
        ...
        componentData.insert(ModuleManagerController::s_entitiesComponentName, QVariant(3));
